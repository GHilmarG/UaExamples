function FE2dTransientPlots(CtrlVar,DTxy,TRIxy,MeshBoundaryCoordinates,GF,dGFdt,coordinates,connectivity,...
    ...
    %plots=' save only ';
    plots='-h(x)-dh/dt(x)-sbB(x)-u(x)-mesh-';
    ...
    if ~isempty(strfind(plots,'-sbB(x)-'))
        %  lower surface
        figure(10000+100*CtrlVar.Implicituvh+10*CtrlVar.TG3+CtrlVar.IncludeTG3uvhBoundaryTerm)
        hold off
        x=coordinates(:,1) ;
        [xsorted,I]=sort(x);
        plot(x(I)/CtrlVar.PlotXYscale,s(I),'.-') ; hold on
        plot(x(I)/CtrlVar.PlotXYscale,b(I),'.-')
        plot(x(I)/CtrlVar.PlotXYscale,B(I),'.-')
    ...
    if ~isempty(strfind(plots,'-h(x)-'))
        %  lower surface
        figure(1000+100*CtrlVar.Implicituvh+10*CtrlVar.TG3+CtrlVar.IncludeTG3uvhBoundaryTerm)
        hold off
        x=coordinates(:,1) ;
        [xsorted,I]=sort(x);
    ...
    if ~isempty(strfind(plots,'-dh/dt(x)-'))
        %  lower surface
        figure(2000+100*CtrlVar.Implicituvh+10*CtrlVar.TG3+CtrlVar.IncludeTG3uvhBoundaryTerm)
        ...
        hold on
        title(sprintf('dhdt(x) at t=%g ',time))
        hold off
    ...
    if ~isempty(strfind(plots,'-u(x)-'))
        %  lower surface
        figure(3000+100*CtrlVar.Implicituvh+10*CtrlVar.TG3+CtrlVar.IncludeTG3uvhBoundaryTerm)
        hold off
        x=coordinates(:,1) ;
        [xsorted,I]=sort(x);
        plot(x(I)/CtrlVar.PlotXYscale,h(I),'.-')
        xlabel('xps (km)') ; ylabel('u (m/yr)') ;
        hold on
        title(sprintf('u(x) at t=%g ',time))
        hold off
