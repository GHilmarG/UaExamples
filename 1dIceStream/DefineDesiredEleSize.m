...
if  contains(UserVar.AdaptMesh,"-Recovery-based error estimate-")
    ...
    if ~all(F.ub==0)
        ...
        cbar=UaPlots(CtrlVar,MUA,F,R,logColorbar=true,FigureTitle="Recovery-based error estimate");
       
        title("Recovery-based error estimate",Interpreter="latex")
        subtitle(sprintf("$\\max(R)=%g, R_{\\mathrm{crit}}=%g$",max(R),Rcrit),Interpreter="latex") 
        title(cbar,"$R$",interpreter="latex")

       

        clim([1 1000])
        
        CM=cmocean('balanced',25,'pivot',Rcrit) ; colormap(CM);
