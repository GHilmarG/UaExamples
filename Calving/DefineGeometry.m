...
switch UserVar.InitialGeometry
    ...
    case "-MismipPlus-" % "Mismip3"  "Constant" ;
        ...
        DataFile="../../Interpolants/MismipPlusThicknessInterpolants.mat" ;
        
        if ~isfile(DataFile)
            
            fprintf("The input file: %s is missing. \n",DataFile)
            fprintf("You might be able to download this from: \n")
            fprintf("https://livenorthumbriaac-my.sharepoint.com/:f:/g/personal/hilmar_gudmundsson_northumbria_ac_uk/EgrEImnkQuJNmf1GEB80VbwB1hgKNnRMscUitVpBrghjRg \n")
            error("DefineGeometry:InputFileMissing","Input file missing")
        end
        ...
        if contains(FieldsToBeDefined,"s")
            ...
            switch CtrlVar.SlidingLaw
                ...
                case "Tsai"
                    load(DataFile,'FsTsai','FbTsai')
                    s=FsTsai(x,y) ;
                    b=FbTsai(x,y) ;
                otherwise
                    load(DataFile,'FsWeertman','FbWeertman')
                    s=FsWeertman(x,y) ;
                    b=FbWeertman(x,y) ;
            end
        else
